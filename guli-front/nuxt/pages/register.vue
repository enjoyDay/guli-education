    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item
          ...
          prop="email"
          :rules="[{ required: true, message: '请输入邮箱账号', trigger: 'blur' },{validator: checkPhone, trigger: 'blur'}]"
        >
          <div>
            <el-input type="text" placeholder="邮箱号" v-model="params.email" />
  ...
  methods: {
    ...
    submitRegister() {
      registerApi.registerMember(this.params).then(response => {
        if (response && response.data.data.code === 20000) {
          //提示注册成功
          this.$message({
            type: "success",
            message: "注册成功"
    ...
    //通过输入邮箱号发送验证码
    getCodeFun() {
      console.log("邮箱号是：", this.params.email);
